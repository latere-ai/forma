...
// Command forma runs a model, measures it, and reports what it is.
//
//	forma run   [--prompt P] [--max-tokens N] [--temp T] [--seed S] [--precision f16|int8|auto] <model-dir>
//	forma bench [--tokens N] [--prompt-tokens N] [--batch N] [--json out.json] <model-dir>
//	forma serve [--addr host:port] [--public] [--precision P] [--context C] [--slots N] [--kv P] [--prefix-cache S] [--batched] [--device D] <model-dir>
//	forma info  <model-dir>
//	forma pull  [--revision R] [--token T] <repo-id>
//
...
// --device auto|cpu|metal and --precision f16|int8|auto. `forma pull` opens none
// and takes neither: it turns a repo id into a directory the other four can be
// pointed at. `forma help` prints the whole surface, and
...
//   - `forma info` computes that choice, specs/005-kv-cache.md §3's cache
...
//     model by the FORMA_MODEL tests in engine_test.go.
...
//     (017-D4).
//   - The batch axis is reported as an axis even though forma does not batch yet.
...
const usage = `
usage: forma <command> [flags] <model-dir>
