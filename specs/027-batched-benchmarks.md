...
`scheduler.go` emit nothing. `forma bench --batch 8` is refused at
`cmd/forma/bench.go:86-91`.
...
| the record is an axis with one point and a note saying why | `cmd/forma/record.go:119-135`, `cmd/forma/bench.go:237-244` |
...
measurable by a user of the library and not only by `cmd/forma`.
...
- `cmd/forma/record.go:242-251` — `hasBreakdown` iterates `{Prefill, Decode}`, so
  a run made only of mixed steps renders `noBreakdownNote` and reports itself as
  unmeasured.
- `cmd/forma/record.go:313-319` — `renderMarkdown` writes two phase sections and
  needs a third.

## 5. How the harness drives a batch

`forma bench --batch N` admits $N$ synthetic conversations into one
...
`forma_decode_step_seconds` would stop moving the moment a request got long. A
...
nothing for `cmd/forma/bench` — which sizes the recorder above its window at
`cmd/forma/bench.go:186` and never drops — and costs one modulo on the hot path.
The ring is the fix.
...
- `cmd/forma/record.go:342-347` — the Markdown warning says the percentiles
...
`recordSchema` (`cmd/forma/record.go:17-22`) goes from `forma.bench/1` to
`forma.bench/2`. The fields added:
...
| `TestParseBenchAcceptsABatch` (**replaces a row**, `cmd/forma/bench_test.go:39`) | `--batch 8` parses. The refusal row leaves `TestParseBenchRefusals`, and a new refusal covers a batch the pool cannot hold |
| `TestRecordSchemaIsAdditive` | the checked-in `forma.bench/1` document decodes into `benchRecord` with every `/1` field populated (§7) |
| `TestNoNoteClaimsAnUnbuiltScheduler` | no string in `cmd/forma` says 008 is drafted or unbuilt. §9 is why this is a test and not a checklist |

## 9. Four strings that describe a Forma that no longer exists
...
| --- | --- | --- |
| `singleBatchNote` | `cmd/forma/record.go:130-135` | "Forma does not batch yet. specs/008-scheduler.md is drafted and unbuilt, so exactly one sequence is ever in flight" |
| the `--batch` refusal | `cmd/forma/bench.go:82-91` | "Forma runs one sequence at a time: specs/008-scheduler.md is drafted and unbuilt, so there is no batched path to measure" |
| `noBreakdownNote` | `cmd/forma/record.go:155-162` | the engine's "public surface (§1) exports no way to set and no way to read" a recorder |
| `noPlanStatsNote` | `cmd/forma/record.go:173-177` | plan compile time per bucket and cache hit rate are absent. **Still true**: `Model.cache` is unexported (`plan.go:86-103`) and nothing reports statistics about it |
...
`cmd/forma/engine.go:79-90` and `cmd/forma/main.go:48-56`.

`cmd/forma/bench_test.go:39` pins the refusal:
...
`singleBatchNote` is **replaced, not deleted**. `cmd/forma/record.go:182-186`
...
- **A comparison against vLLM or sglang.** `comparisonNote`
  (`cmd/forma/record.go:137-141`) stands unchanged.
...
| 027-D6 | the schema string becomes `forma.bench/2` and `/2` is additive over `/1` | keeping `/1` and adding fields | a version nothing increments is decoration, and 017-D6 makes the string the first thing a gate reads. Additive-only is what lets a `/1` reader decode a `/2` document, and a checked-in `/1` fixture is what pins it rather than review |
| 027-D7 | an axis that is now measurable keeps its note field, with new text | deleting `singleBatchNote` once the scheduler ships | `cmd/forma/record.go:182-186`'s argument does not expire: a record that cannot distinguish "not measured" from "does not exist" stops gating the axis silently. The note stops saying Forma cannot batch and starts saying what the arrival pattern hides |
