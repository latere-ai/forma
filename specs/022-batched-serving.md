...
"shares with nobody rather than with everybody". Of `Pool.route`'s choice of
session that is true by construction: `e.key != key` skips the entry. Of the
blocks a pooled session reaches under `CacheProcess` it was not, because
routing there matches nothing and the key goes to the block pool as the salt,
so the pooled engine had the hole described next until it minted the same way
(`Pool.salt`). So did a `Session` opened directly on the model, and with it
`server.Wrap`'s engine, which opens one per request: `Model.NewSession` kept an
absent `WithCacheSalt` as the empty salt, so every unsalted session under
`CacheProcess` hashed into one domain until it minted one per session
(`blockPool.mintSalt`).
...
`--prefix-cache off` remains the way to get the old behavior, and it now also
...
| 022-D3 | One driver goroutine calls `Step`, samples every slot, and sends `forma.Event` values on per-slot bounded channels. The request goroutine never sees logits. | A goroutine per slot calling `Step`. `Scheduler.Step` serializes on its own mutex and `Batch.Step` takes `Model.mu` (007-D9), so B goroutines would interleave rather than batch, which is the current behavior with more machinery. | No per-token copy: `Produced.Logits` is read on the goroutine it arrived on, inside its "until that slot steps again" lifetime (`batch.go:290`). |
...
| 022-D5 | Cancellation is honored at the step boundary: the slot is marked dead, the in-flight dispatch finishes, `Scheduler.Finish` releases it and its blocks return to the pool. | Aborting the dispatch. accel has no cancel on a submitted queue, and 007-D9 records that submitting against a queue mid-flight gets a failed fence rather than a race. | The worst-case hold on a slot is one step. The 499 is written by the handler immediately and independently (`server/generate.go:56`). |
...
| 022-D8 | A request with no `cache_salt` gets a per-request salt, so it shares with nothing. The pooled engine under `CacheProcess` does the same, one salt per lease (`Pool.salt`), because its routing matches nothing there and so never stood in for this. A `Session` opened without `WithCacheSalt` does the same, one salt per session (`blockPool.mintSalt`), so its own turns still share with each other and with nothing else. | Passing the empty string through. Under `ScopeProcess` the seed's domain is empty (`internal/prefix/prefix.go:290`), so every unsalted request hashes into one domain, which is the opposite of what 019-D3 and [016 §7.1](016-prefix-cache.md) promise. | Cross-request block reuse happens only where a caller asked for it, so batching can be the default without [016-D7](016-prefix-cache.md) becoming one. |
