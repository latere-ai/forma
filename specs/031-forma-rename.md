...
- Rename `cmd/tgo` to `cmd/forma`. Use `FORMA_CACHE`, `FORMA_MODEL`, and
  `FORMA_REQUIRE_METAL`, a `forma` cache directory, `X-Forma-Loss`, and
  `forma.bench/1`. Document the names a caller must migrate. Existing checkpoint
...
through their existing behavioral tests. Add a CLI naming regression test and
verify that it fails before the rename. After publication, check repository
