...
"Tokens per second" hides the thing Forma is betting on. A decode step is

$$t_\text{step} = t_\text{host} + t_\text{submit} + t_\text{device} + t_\text{readback}$$

and Forma's claim is that a compiled language wins $t_\text{host}$ while accel
decides $t_\text{device}$. A single throughput number cannot tell those apart,
so it cannot tell whether a regression is Forma's fault or accel's — which is the
...
1. **Publish losses.** Every row, including the ones Forma is behind on. A
...
6. **The comparison is reproducible from the repository**: one command, the
   ...
   (`cmd/forma/record.go:60-66`). No prompt set is checked in: `forma bench` builds
   its prompt at run time from one repeated word (`cmd/forma/bench.go:121`), so
...
1. **Submit is 15.61% of a decode step and 1.12% of a prefill.** Per-dispatch
   ...
   **Filed as [accel#21](https://github.com/golang-design/accel/issues/21) and
   ...
   instrument could: not a per-*step* submission cost as Forma assumed, but a
   ...
   are where Forma should win, so measuring itself losing 15% of a step to
...
they flagged as deliberate is not what Forma waits on, and there is nothing
...
fact about kernel maturity dressed as a fact about Forma. The row waits for
[011 M13](011-sequencing.md), and what would make it meaningful first is the
submit overhead in finding 1 — the one axis [000 §11](000-decisions.md) says Forma
should win.

## 5. Where the numbers go

`forma bench` writes a Markdown table and a JSON record. The JSON is what a
...
Package `bench` is Forma's decode instrument. It records the four terms of every
step, aggregates them as percentiles, and `forma bench` renders the aggregate as a
Markdown table and a versioned JSON record. The package landed in Wave 1
(2026-08-24); `WithRecorder` and the `forma bench` command landed in Wave 4
...
| 3 | four of six rows: throughput, host share, cold and warm TTFT, resident memory, and the readback share in production | `cmd/forma/record.go:83-89`, `:109-117`; `server/metrics.go:124-136`, `server/generate.go:335` |
| 4 | rules 2, 3 and 4 in the harness: the conditions beside every number, warm up then `Reset`, the prompt recipe in the record | `cmd/forma/record.go:71-89`, `cmd/forma/bench.go:205-216`, `:113-133` |
...
| 5 | the Markdown table on stdout, and the `forma.bench/1` record on `--json`, byte-stable | `cmd/forma/bench.go:150-164`, `cmd/forma/record.go:22`, `:260` |
| 6 | all five rows, and the breakdown twice: through the real engine on synthetic weights, and through `cmdBench` against a fake one | `bench/bench_test.go:44`, `:81`, `:134`, `:206`; `engine_test.go:824`; `cmd/forma/bench_test.go:178` |
| DR | D1, D2, D3, D4 and D7 built and tested | `bench/report.go:41-45`, `:184`; `bench/bench.go:106-119` with `engine_test.go:880`; `cmd/forma/record.go:71-89`; `options.go:161` |
...
- §4 rule 1 is enforced as *name every unmeasured axis*, not as *publish every
  ...
  (`cmd/forma/record.go:131`, `:138`, `:156`, `:173`) and
  `cmd/forma/record_test.go:392` fails if a note is dropped. A rule about rows
...
- The step p50/p90/p99 in the Markdown table, §4.1's own figures included, is
  the sum of four independently sorted quantiles (`cmd/forma/record.go:380`
...
- **Two stale strings in `cmd/forma`**, and they are [027](027-batched-benchmarks.md)'s:
  ...
  (`cmd/forma/bench.go:88`) both say "specs/008-scheduler.md is drafted and
  unbuilt" while 008 is `complete`, and `cmd/forma/bench_test.go:40` asserts that
  ...
  **The other pair is corrected** (2026-08-28). `noBreakdownNote` and
  `cmd/forma/main.go`'s discrepancy list both said specs/007-engine.md §1 exports
...
- **017-D6's regression gate**: [028](028-performance-gate.md). The record is
  written and shaped for a checker that does not exist (`cmd/forma/record.go:205`),
  there is no benchmark counterpart to the coverage gate, and neither
  `.github/workflows/ci.yml` nor `ci-metal.yml` runs `forma bench`.
- **Plan compile time per bucket, and plan-cache hit rate**: §3's last row waits
  ...
  cannot be separated from it (`cmd/forma/record.go:173` `noPlanStatsNote`).
- **The vLLM and sglang comparison**: gated by [011 M13](011-sequencing.md), and
  ...
  addressed (`cmd/forma/record.go:138` `comparisonNote`).

## Decision record

| id | decision | rejected | consequence |
| --- | --- | --- | --- |
| 017-D1 | every measurement is a host/submit/device/readback breakdown | one throughput number | a regression can be attributed to Forma or to accel, which is the question the project exists to answer |
